...
function getHealth(health, initialHealth) {
  return Math.min(health, initialHealth)
}

function solution(bandage, initialHealth, attacks) {
  const BANDAGE_CAST_TIME = bandage[0];
  const BANDAGE_RECOVERY_RATE = bandage[1];
  const BANDAGE_ADDITIONAL_RECOVERY = bandage[2];
  ...
  while(time <= lastAttack && health > 0) {
    ...
    if(attackIndex < attacks.length && time === attacks[attackIndex][0]) {
      ...
      if(recoveryCount === BANDAGE_CAST_TIME) {
        health = getHealth((health + BANDAGE_RECOVERY_RATE + BANDAGE_ADDITIONAL_RECOVERY), initialHealth)
        recoveryCount = 0
      } else {
        //증가된 연속성공이 시전 시간과 같지 않다면 기존 health에 초당 회복량만 더하기(최대 initialHealth)
        health = getHealth((health + BANDAGE_RECOVERY_RATE), initialHealth)
      }
    }
    //time 1 증가
    time++
